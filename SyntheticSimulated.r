...
#switching to rate scale for no bed hold facilities
...
fileName = sprintf("simSynth_%d_%d.csv",minFacID,maxFacID) 
...
if(minFacID == 1)
{
    noBedHoldF = noBedHoldF[-which(noBedHoldF$facid == 1865),]
}
...
if(minFacID == 301)
{
    noBedHoldF = noBedHoldF[-which(noBedHoldF$facid == 1918),]
}
...
#prepare dataset to be used by synthetic control function
 ...
 #running synthetic control for facility j
  dataprep.out <- dataprep(
               foo = newFile,
               predictors = c("nhmod","died_qtr"),
               predictors.op = "mean",
               time.predictors.prior = 1:numBefore,
               special.predictors = list( list("acute_qtr", 1:numBefore , "mean")),
               dependent = "acute_qtr",
               unit.variable = "facid",
  ...
  for(i in 1:numControls)
  {
   ...
   #running synthetic control command
   dataprep.out <- dataprep(
